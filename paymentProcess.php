...
$merchantId = "1225785";
$merchantSecret = "NTEzNjM0MjE0MzEyMTY4MTE0NzIwMzkxMzcwNDQzNzk5Njk0OTU4";
$items = "";
$netTotal = 0;
$currency = "LKR";
$orderId = uniqid();
...
for ($i = 0; $i < sizeof($stockList); $i++) {
    $rs2 = Database::search("SELECT * FROM `stock` INNER JOIN `product` ON 
            `stock`.`product_id` = `product`.`id` WHERE `stock`.`stock_id` = '" . $stockList[$i] . "'");
